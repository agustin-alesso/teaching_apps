...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Playing with Principal Components"),
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            p("3. Define the correlation matrix matrix"),
            fluidRow(
            ...
            fluidRow(
                column(1, "Y1"),
                column(3, numericInput(inputId = "r12", label = NULL, value = 1, step = 0.05, min = -1, max = 1)),
                column(3, numericInput(inputId = "r13", label = NULL, value = 1, step = 0.05, min = -1, max = 1)),
                column(3, numericInput(inputId = "r14", label = NULL, value = 1, step = 0.05, min = -1, max = 1))
            ),
            fluidRow(
                column(1, "Y2"),
                column(3, offset = 3, numericInput(inputId = "r23", label = NULL, value = 1, step = 0.05, min = -1, max = 1)),
                column(3, numericInput(inputId = "r24", label = NULL, value = 1, step = 0.05, min = -1, max = 1))
            ),
            fluidRow(
                column(1, "Y3"),
                column(3, offset = 6, numericInput(inputId = "r34", label = NULL, value = 1, step = 0.05, min = -1, max = 1))
            ),
            numericInput(inputId = "n",  label = "sample size", value = 30),
            checkboxInput(inputId = "cormat", label = "Use correlation matrix", value = T),
...
server <- function(input, output) {
    ...
    #output$pop_r <- as.matrix(R)
    #output$pop_vcov <- Sigma
    
    # Simulate data
    data <- reactive({
        Y <- mvrnorm(n = input$n, mu = mu(), Sigma = Sigma())
        Y <- data.frame(Y)
